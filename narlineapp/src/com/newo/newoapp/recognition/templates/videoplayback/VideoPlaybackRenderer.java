...
public class VideoPlaybackRenderer implements GLSurfaceView.Renderer {
	...
	public VideoPlaybackRenderer() {
		...
		mVideoPlayerHelper = new VideoPlayerHelper[ScanActivity.NUM_TARGETS];
		mMovieName = new String[ScanActivity.NUM_TARGETS];
		mCanRequestType = new MEDIA_TYPE[ScanActivity.NUM_TARGETS];
		mSeekPosition = new int[ScanActivity.NUM_TARGETS];
		mShouldPlayImmediately = new boolean[ScanActivity.NUM_TARGETS];
		mLostTrackingSince = new long[ScanActivity.NUM_TARGETS];
		mLoadRequested = new boolean[ScanActivity.NUM_TARGETS];
		mTexCoordTransformationMatrix = new float[ScanActivity.NUM_TARGETS][16];

		// Initialize the arrays to default values
		for (int i = 0; i < ScanActivity.NUM_TARGETS; i++) {
	...
	public native void setVideoDimensions(int target, float videoWidth,
			float videoHeight, float[] textureCoordMatrix);

	/**
	 * Native call to indicate whether the rendering should display a keyframe
	 * or the video data
	 */
	private native void setStatus(int target, int value);

	/**
	 * Native function that informs whether the target is currently being
	 * tracked.
	 */
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		...
		for (int i = 0; i < ScanActivity.NUM_TARGETS; i++) {
			...
			if (mVideoPlayerHelper[i] != null) {
				...
				// the texture id created in native
				...
				if (!mVideoPlayerHelper[i]
						.setupSurfaceTexture(getVideoTextureID(i)))
