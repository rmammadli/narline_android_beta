...
public class VideoPlayerHelper implements OnPreparedListener,
	...
	private String qcTargetId;
	private boolean  wasPointsAdded;
	
	public void setWasPointAdded(boolean val)
	{
		wasPointsAdded = val;
	}
	
	public VideoPlayerHelper(ScanActivity scanActivity)
	{
		
		this.scanActivity = scanActivity;
		wasPointsAdded = false;
	}
	
	public void setQcTargetId(String qcTargetId)
	{
		this.qcTargetId = qcTargetId;
	}
	...
	public boolean load(String filename, MEDIA_TYPE requestedType,
		...
		Log.d("videoLink", "videoLink : " + filename);
																						...
																						// device
			{
				if (mSurfaceTexture == null) {
					...
					try {
						...
						// mParentActivity.getAssets().openFd("VuforiaSizzleReel_1.m4v");
	...
	public boolean play(boolean fullScreen, int seekPosition) {
		...
		if (fullScreen) {
			...
			mPlayerHelperActivityIntent.putExtra("requestedOrientation",
					mParentActivity.getRequestedOrientation());
    ...
    private Runnable mUpdateTimeTask = new Runnable() {
           public void run() {
               ...
               DebugLog.LOGD("  getCurrentPosition: " + currentDuration);
	...
	/**
	 * The following functions are specific to Android and will likely not be
	...
	public void onBufferingUpdate(MediaPlayer arg0, int arg1) {
		DebugLog.LOGD("onBufferingUpdate " + arg1 + "  getCurrentPosition: " + getCurrentPosition());

		if(getCurrentPosition()>10000 && !wasPointsAdded)
		{
			wasPointsAdded = true;
			scanActivity.addPoints(qcTargetId);
		}
	...
	public boolean setupSurfaceTexture(int nativeTextureID) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
			...
			// We have to give it a texture id of an already created (in native)
			// // OpenGL texture
			mSurfaceTextureLock.lock();
