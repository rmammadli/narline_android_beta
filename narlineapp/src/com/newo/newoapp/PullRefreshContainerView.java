...
/**
 * A container for a ListView that can be pulled to refresh. This will create a
 * ListView and refresh header automatically, but you can customize them by
 * using {@link #setList(ListView)} and {@link #setRefreshHeader(View, int)}
 * <p>
 * To use, put this where you would normally put your ListView. Since this does
 * not extend ListView, you must use {@link #getList()} to modify the list or
 * provide your own.
 * <p>
 * To get the actions of the list, use a {@link OnChangeStateListener} with
 * {@link #setOnChangeStateListener(OnChangeStateListener)}. If you want to
 * change how the refresh header looks, you should do it during these state
 * changes.
 */
public class PullRefreshContainerView extends LinearLayout {
	...
	public interface OnChangeStateListener {
		 ...
		 * 
		 * @param container
		 *            The container that contains the header
		 * @param state
		 *            The state of the header. May be STATE_IDLE, STATE_READY,
		 *            or STATE_REFRESHING.
		 */
	...
	/**
	 * State of the refresh header when it is doing nothing or being pulled down
	 * slightly.
	 */
	public static final int STATE_IDLE = 0;

	/**
	 * State of the refresh header when it has been pulled down but not enough
	 * to start refreshing, and has not yet been released.
	 */
	public static final int STATE_PULL = 1;

	/**
	 * State of the refresh header when it has been pulled down enough to start
	 * refreshing, but has not yet been released.
	 */
		...
		if ((REFRESH_VIEW_HEIGHT / 4 <= mCurRefreshViewHeight)
			...
			if (height > REFRESH_VIEW_HEIGHT) {
				height = (int) (REFRESH_VIEW_HEIGHT + (height - REFRESH_VIEW_HEIGHT)
						* REFRESH_VIEW_HEIGHT * 1.0f / height);
			}
	 ...
	 * <p />
	 * The header view is the view at the top that will show while the list is
	 * refreshing. Usually, this will be a simple rectangle that says
	 * "refreshing" and the like.
	 * <p />
	 * 
	 * @param headerView
	...
	public void setRefreshHeader(View header) {
		...
		if (header == null) {
			throw new RuntimeException(
					"Please supply a non-null header container.");
		}
	...
	/**
	 * Notifies the pull-to-refresh view that the refreshing is complete. This
	 * will hide the refreshing header.
	 */
	...
	/**
	 * @param listener
	 *            the listener to be notified when the header state should
	 *            change
	 */
